...
**IMPORTANT NOTE**: We try our best to make this specification as clear as possible and cover all the problems we met during our testing. However, it is not uncommon that we could still miss important details in this specification. If there is anything unclear, you should contact the instructors and SA Yuhang Wang in QQ group immediately, rather than guessing what you are required to do.
...
**NOTE:** We will use Mininet to build different network topologies to test the correctness of the os-ken controller you write. You are thus required to ensure your code works properly using customized network topologies.

## Environment Setup
The environment setup consists of two main steps. First, install Mininet, and second, install the experimental framework we provide (including os-ken).
...
For a more convenient development experience, we recommend using the Remote - SSH extension in VS Code to connect to the virtual machine and develop the project remotely.
...
The project repository is located at Blackboard. After downloading the repository, install the Python package dependencies with the following command.
...
# Check if os-ken is installed successfully
osken-manager --version
# If you see the version information of osken-manager, the installation is successful.
```
...
## Tasks
The basic part of this project includes three parts: a simple DHCP server, implementation of the shortest path switching algorithm and implementation of the firewall. To simplify the experiment, we have imposed the following restrictions on the network topology structure.
...
**If you are unfamiliar with the terms such as action and flow table, please refer to our slides, the course textbook, and the documentation of os-ken and the relevant information of the Openflow protocol.**
...
- `firewall.py`: Implements the firewall module, including parsing firewall rules and generating flow entries. `controller.py` uses this module to install firewall rules into the switches' flow tables.
- `tests`: Scripts for building mininet networks to test dhcp, switching and firewall functions.
...
We have set the default IP allocation range in `dhcp.py`. You can check whether the two hosts have been assigned IP addresses by using the commands `h1 ifconfig` and `h2 ifconfig`.
As long as the assigned IP addresses are within the range of `192.168.1.2` to `192.168.1.99`, we consider the basic DHCP functionality to be correctly implemented.
...
We can leverage the centralized SDN architecture to implement firewall functionality. Instead of filtering packets on each host, the controller parses firewall rules and installs corresponding flow entries into the switches. When a packet matches a firewall rule, the switch drops the packet directly according to the installed flow entry.

#### Implementing Firewall Rules

- The firewall rules are defined in `firewall_rules.json`. Each rule may specify the source IP address, destination IP address, transport protocol, source port, destination port, and action. In this project, the firewall mainly supports `deny` rules.

- When the controller starts, `firewall.py` loads the firewall rules from the rule file. If the rule file does not exist, the firewall module uses a set of default rules. Each rule is represented as a `FirewallRule` object, which contains fields such as `src_ip`, `dst_ip`, `proto`, `src_port`, `dst_port`, and `action`.

- The firewall module normalizes the rule fields before installing them. For example, empty values, `*`, and `any` are treated as wildcard fields. Protocol names such as `icmp`, `tcp`, and `udp` are converted into their corresponding IP protocol numbers.

- A firewall rule is installed as a drop rule. Specifically, the flow entry matches packets according to the specified IP addresses, protocol, and ports, and uses an empty action list. Therefore, when a packet matches the rule, the switch drops the packet instead of forwarding it. In this way, the firewall achieves the goal of blocking specified network packets.

- For each switch in the network, `controller.py` calls the firewall module to install the firewall rules. The firewall module converts each `deny` rule into a high-priority OpenFlow flow entry and installs it into the switch’s flow table.

In this implementation, the firewall is enforced directly by the switches through OpenFlow flow entries. The controller is responsible for parsing firewall policies and installing the corresponding rules, while the switches perform packet filtering at runtime.

#### Testing firewall

The `test_firewall.py` script builds a simple Mininet topology with three hosts and one switch:
...
h3 ---/
```
To focus on the implementation of the firewall functionality, the IP addresses of the three hosts are manually configured in the test script. The goal of this test is to check whether the firewall can block network packets according to the specified rules.

The script starts two HTTP servers on h2, one on port 80 and the other on port 8080. It then performs four tests:

h1 -> h2 ICMP: should fail because ICMP traffic from h1 to h2 is blocked.

h1 -> h3 ICMP: should pass because no firewall rule blocks this traffic.

h1 -> h2 TCP/80: should fail because TCP traffic from h1 to h2 on port 80 is blocked.

h1 -> h2 TCP/8080: should pass because port 8080 is not blocked.

To use `test_firewall.py`, first you need to execute the following command in one terminal:
```
osken-manager --observe-links controller.py
```
In another terminal, execute the following command:
...
If the results match the expected behavior below, the firewall module is working correctly.
<p align="center">
...
- DHCP: 20 pts
- Shortest path switching: 40 pts
- Firewall: 20 pts
- Report: 10 pts
- Bonus: Up to 20 pts
...
### Useful Documents
1. os-ken's API documentation https://docs.openstack.org/os-ken/latest/
